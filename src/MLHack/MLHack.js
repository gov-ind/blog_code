  ...
  render() {
    return (
        <div className='content'>
          ...
          <p>
          This week, while playing <a href="https://datahack.analyticsvidhya.com/contest/job-a-thon-april-2022">Analytics Vidhya's Jobathon</a>, an ML hackathon, I stumbled upon a way to "hack" the public leaderboard and climb to the top of the rankings. <b>Disclaimer: </b>The public leaderboard is not actually used to decide the winner of the competition, so this trick is not abusive, it's really just a fun observation.
         </p>
         <p>Some background about ML hackathons: In these events, the idea is to build a machine learning model that is trained on some input data (provided by the organizers) and is then used to make some predictions on some test data (again, provided by the organizers). These predictions are compared with the actual values using some sort of evaluation metric (typically, the RMSE or Root Mean Squared Error), and the models are ranked in a public leaderboard sorted by the metric in ascending order.
          </p>
          ...
          <p>It's easy to solve this system of linear equations. Subtracting the two, we get:</p>
          <div className='center-eq'>
            <Tex>{t8}</Tex>
          </div>
