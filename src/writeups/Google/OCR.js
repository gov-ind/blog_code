  ...
  render() {
    return (
      <div className="content">
        ...
        />
        <p>
          <b>Description. </b>In this ML challenge at Google's CTF, the aim is
          to train a tiny server-side neural network to read the flag from a set
          of <Tex>16 \times 16</Tex> images, where each image holds one
          character of the flag. Here's the server-side script:
        </p>
        <Code>{code1}</Code>
        <p>
          <b>Neural networks crash course. </b>A neural network is a circuit
          that attempts to model an arbitrary function{" "}
          <Tex>{"f(" + X + ")"}</Tex> for some input <Tex>{X}</Tex>. Here the
          input <Tex>{X}</Tex> is a <Tex>16 \times 16 \times 3</Tex> image and
          the output is a single character, the network's guess for what that
          image is.
        </p>
        <p>
