  ...
  useEffect(() => {//debugger;
    ...
    if (scrollDiv && scrollDiv.current) {
      const handleScroll = () => {
        ...
        let i = 0;
        while (cumHeights[i] < scrollDiv.current.scrollTop && i < cumHeights.length - 1) {
          i++;
        }
        //replay(i, videoRef);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', 'flex': 1 }}>
      ...
      <div className='row full' style={{ overflow: 'hidden' }}>
        <div ref={scrollDiv} className='column w-50pc full' style={{ 'overflowY': 'auto' }}>
          {pages_.map((page, ix) => <div id={'childDiv' + ix}>{page}</div>)}
        </div>
