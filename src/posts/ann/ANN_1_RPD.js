...
const pages = {
  0: <div className='p-r-1 full ann-rpd-sidepanel'>
    <p>
    An Approximate Nearest Neighbours (ANN) search algorithm is the secret sauce that makes vector databases so fast. In this writeup, the first of a three-part blog post that peeks under the hood of the ANN algorithms used by some state-of-the-art vector databases, we'll look at K-D trees, a basic Exact Nearest Neighbour search algorithm, and Random Projection Trees, an ANN algorithm used by FLANN and Spotify's Annoy.
    </p>
    <p>
    This is also the first time I've tried incorporating videos into my posts, so this experimental ser-interface can seem a bit unconventional. On this left panel, I'll be transcribing what's going on in the <a href="https://www.3blue1brown.com/">3b1b</a>-style video embedded in the right panel. Use the first and last buttons below to navigate to the previous and next page, and the middle button to replay the current animation. Go ahead and click the button on the right to get started.
    </p>
  </div>,
  1: <div className='p-r-1 full ann-rpd-sidepanel'>
    <p>
    To keep things simple, we'll start with a one-dimensional example, a set of points scattered along the number line. Assume that we a have a point (the red one) whose <span className='code-block'>k</span> nearest neighbours we wish to find.
    </p>
    <p>
    The naive approach is to calculate the distance between this point and each point on the number line and return the <span className='code-block'>k</span> points with the smallest distances. Unfortunately, this doesn't scale very well in real-world machine learning pipelines that output up to many millions of vectors. We can do better.
    </p>
  </div>,
  2: <div className='p-r-1 full ann-rpd-sidepanel'>
    <p>
  ...
  4: <div className='p-r-1 full ann-rpd-sidepanel'>
    <p>
    We'll use K-Means clustering to find clusters in this example (Annoy and FLANN use K-Means too, amongst other techniques). A quick refresher of how K-Means works: First, pick <span className='code-block'>K</span> points (here we set <span className='code-block'>K</span> to 2 because at each partition, we're looking to find two clusters). Next, pick a random point from the dataset and calculate its distance from each of the two points. Finally, move the closer of the two points towards the randomly picked point. Repeat the above three steps till the two centroids almost converge.
    </p>
    <p>
  ...
  5: <div className='p-r-1 full ann-rpd-sidepanel'>
  <p>
  ...
  <p>
    The tree that results from this kind of "intelligent" partitioning is called a K-D Tree. It's these carefully chosen cluster centroids (or indexes/indices, as they are commonly called) that make lookups fast in K-D Trees.
  </p>
</div>,
  6: <div className='p-r-1 full ann-rpd-sidepanel'>
  <p>
    Can K-D Trees be used for two-dimensional data? Yes, but alas, it's not always effective. If our data is evenly spread out, it works well; Otherwise, as we'll soon see, K-D trees have a drawback.
  </p>
  <p>
  ...
  7: <div className='p-r-1 full ann-rpd-sidepanel'>
    <p>
  ...
  8: <div className='p-r-1 full ann-rpd-sidepanel'>
    <p>
  ...
  9: <div className='p-r-1 full ann-rpd-sidepanel'>
    <p>
      Unfortunately, the greedy search only worked because our data was symmetric. If it were skewed such that each cluster had contrasting covariances, there's no way for our tree to account for each all the directions of variance at each split.
    </p>
    <p>
  ...
  10: <div className='p-r-1 full ann-rpd-sidepanel'>
    <p>
    ...
    <p>
      Another way to think about this curse is as follows: For 2-dimensional data, we need at least 4 data samples for a K-D Tree to be effective. In other words, for <span className='code-block'>n</span>-dimensional data, since each node splits the data in two, we need at least <Tex>2^n</Tex> data samples to see any performance gains. So unless you've got <Tex>{eq1}</Tex> samples, a K-D Tree isn't going to capture the geometry of your 40-dimensional data very well.
    </p>
  </div>,
  11: <div className='p-r-1 full ann-rpd-sidepanel'>
    <p>
    ...
    <p>
      Here's how this would proceed: First, travel to both the children of the root node and push both of them into a queue, with the top-right node being pushed first as it's closer to the orange dot.
    </p>
  </div>,
  12: <div className='p-r-1 full ann-rpd-sidepanel'>
    <p>
      Next, pop the first node (here, the top-right node) off the queue and traverse each of its children, pushing each into the queue, which, recall, already has the bottom-left node.
    </p>
    <p>
      Depending on its distance from the orange dot, one of these children may be placed ahead of the bottom-left node while the rest will be placed after it. In either case, we only intend on exploring up to a depth of two for this toy example, so we won't be exploring beyond these two nodes.
    </p>
    <p>
      Consequently, the only depth-1 node to explore is the bottom-left node. Once each of its children are traversed, the search is done, and we'll find that one of its children is rightfully (leftfully?) the closest to the orange dot.
    </p>
  </div>,
  13: <div className='p-r-1 full ann-rpd-sidepanel'>
    <p>
      Note that the complexity of this algorithm depends on the quality of our partitions. Had we been luckier with our first partition, we would've had to search through only two nodes instead of three. Is there any way to improve our luck? Perhaps if we make multiple sets of partitions, or in other words, a forest of K-D Trees, there's a good chance of at least one tree being "good".
    </p>
  </div>,
  14: <div className='p-r-1 full ann-rpd-sidepanel'>
    <p>
      These K-D Tree forests, also called Random Projection Trees (RP Trees), are what Annoy and FLANN use. For each tree in the RP Tree, you could either choose the location of the partition to be random (like Annoy) or add a bit of jitter to a partition that divides the subspace in two (like FLANN).
    </p>
    <p>
  ...
  15: <div className='p-r-1 full ann-rpd-sidepanel'>
      <p>
        Although there's a lot of implementation details and performance optimizations I've skipped, the high-level idea behind an ANN is what I've presented here.
      </p>
      <p>
        In Part 2, we'll look at Heirarchichal Navigable Small Worlds (HNSWs), a graph-based extension of K-D Trees, and the Nearest Neighbour Descent, an algorithm to construct approximate neighbourhood graphs.
      </p>
    </div>
}
const mnPage = 0;
