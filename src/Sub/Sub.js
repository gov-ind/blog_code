  ...
  render() {
    return (      
        <div className='content'>
          ...
          <div><b>Description. </b>In this challenge, we're given a script that runs input passed to it through Python's <span className='code-block'>string.replace</span> recursively. The aim is to craft a set of inputs which when passed to this script perform arbitrary tasks, ranging from simple pattern-matching to a full-blown binary adder.
          </div>
          <Code>{code1}</Code>
          <p>
          The format of the input accepted by <span className='code-block'>test_substitution</span> is straightforward: It accepts input of the form <span className='code-block'>pattern1=>pattern2</span>, and recursively replaces occurences of <span className='code-block'>pattern1</span> in the source string with <span className='code-block'>pattern2</span>. Let's now dive in to the levels and see an example.
          </p>
          <div className='space'><b>Level 1.</b>In the first level, we're asked to change something like <span className='code-block'>0000000000initial00000000</span> to <span className='code-block'>0000000000target00000000</span>. Easy enough: A rule like <span className='code-block'>initial=>target</span> will do.
          </div>
          ...
          <p>The most natural thing to do is to start detecting from the middle because (as I wrongly assumed) all palindromes reflect around the middle, and then verify that each character to the left and right of this middle section is the same. However, this approach does not work for many reasons. For starters, odd-length palindromes don't reflect around the middle, and detecting them requires more than the 100 substitution limit this level enforces.
          </p>
